import Navigation from "@/components/Navigation";
import Footer from "@/components/Footer";

const Bridal = () => {
  return (
    <div className="m-0 p-0">
      <Navigation />
      <main className="m-0 p-0">
        {/* Hero Section for Bridal Collection */}
        <section className="relative h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-pink-100">
          <div className="absolute inset-0 bg-black/20"></div>
          <div className="relative z-10 text-center space-y-6">
            <h1 className="text-6xl lg:text-8xl font-serif font-bold text-white">
              <span className="text-cultural">BRIDAL</span>
              <br />
              <span className="text-foreground">COLLECTION</span>
            </h1>
            <p className="text-white text-xl lg:text-2xl font-light tracking-widest opacity-90">
              Your Dream Day Deserves Dream Wear
            </p>
          </div>
        </section>

        {/* Bridal Collection Categories */}
        <section className="section-padding bg-white">
          <div className="container-premium">
            <div className="text-center space-y-6 mb-16">
              <h2 className="text-4xl lg:text-6xl font-serif font-bold">
                <span className="text-cultural">Discover Our</span>
                <br />
                <span className="text-foreground">Bridal Collection</span>
              </h2>
              <p className="text-lg text-gray-600 max-w-3xl mx-auto">
                From traditional bridal sarees to contemporary lehengas, explore our magnificent collection 
                that makes every bride's dream come true on her special day.
              </p>
            </div>

            {/* Bridal Collection Categories Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {/* Bridal Sarees */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-red-100 to-pink-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-red-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">👰</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Bridal Sarees</h3>
                  <p className="text-gray-600 mb-4">Magnificent bridal sarees</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹45,999</p>
                </div>
              </div>

              {/* Bridal Lehengas */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-purple-100 to-indigo-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-purple-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">👑</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Bridal Lehengas</h3>
                  <p className="text-gray-600 mb-4">Royal bridal lehengas</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹55,999</p>
                </div>
              </div>

              {/* Bridal Anarkalis */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-amber-100 to-orange-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-amber-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">💫</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Bridal Anarkalis</h3>
                  <p className="text-gray-600 mb-4">Elegant bridal anarkalis</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹35,999</p>
                </div>
              </div>

              {/* Reception Wear */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-pink-100 to-rose-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-pink-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">🎉</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Reception Wear</h3>
                  <p className="text-gray-600 mb-4">Glamorous reception collections</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹25,999</p>
                </div>
              </div>

              {/* Mehendi Wear */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-green-100 to-emerald-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-green-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">🌿</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Mehendi Wear</h3>
                  <p className="text-gray-600 mb-4">Traditional mehendi outfits</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹15,999</p>
                </div>
              </div>

              {/* Haldi Wear */}
              <div className="group cursor-pointer">
                <div className="bg-gradient-to-br from-yellow-100 to-amber-200 rounded-lg p-8 text-center hover:shadow-lg transition-all duration-300">
                  <div className="w-24 h-24 bg-yellow-300 rounded-full mx-auto mb-4 flex items-center justify-center">
                    <span className="text-2xl">🌻</span>
                  </div>
                  <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">Haldi Wear</h3>
                  <p className="text-gray-600 mb-4">Beautiful haldi ceremony wear</p>
                  <p className="text-lg font-semibold text-cultural">Starting ₹12,999</p>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Why Choose Our Bridal Collection */}
        <section className="section-padding bg-gray-50">
          <div className="container-premium">
            <div className="text-center space-y-6 mb-16">
              <h2 className="text-4xl lg:text-6xl font-serif font-bold">
                <span className="text-cultural">Why Choose</span>
                <br />
                <span className="text-foreground">Our Bridal Collection</span>
              </h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="text-center space-y-4">
                <div className="w-16 h-16 bg-cultural rounded-full mx-auto flex items-center justify-center">
                  <span className="text-2xl text-white">👑</span>
                </div>
                <h3 className="text-xl font-serif font-bold">Royal Craftsmanship</h3>
                <p className="text-gray-600">Each bridal piece is crafted with royal precision, featuring intricate embroidery and premium materials.</p>
              </div>

              <div className="text-center space-y-4">
                <div className="w-16 h-16 bg-cultural rounded-full mx-auto flex items-center justify-center">
                  <span className="text-2xl text-white">💎</span>
                </div>
                <h3 className="text-xl font-serif font-bold">Luxurious Details</h3>
                <p className="text-gray-600">Adorned with zari work, stone embellishments, and hand-embroidered motifs for a luxurious finish.</p>
              </div>

              <div className="text-center space-y-4">
                <div className="w-16 h-16 bg-cultural rounded-full mx-auto flex items-center justify-center">
                  <span className="text-2xl text-white">🌟</span>
                </div>
                <h3 className="text-xl font-serif font-bold">Dream Day Perfection</h3>
                <p className="text-gray-600">Designed to make every bride feel like a queen on her most special day.</p>
              </div>
            </div>
          </div>
        </section>

        <Footer />
      </main>
    </div>
  );
};

export default Bridal; 